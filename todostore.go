...
type Config struct {
	// The address of the Todostore API.
	Address string
...
func DefaultConfig() *Config {
	config := &Config{
		Address:    os.Getenv("TODOSTORE_ADDRESS"),
...
// Client is the Todostore API client. It provides the basic
// connectivity and configuration for accessing the Todostore API.
type Client struct {
	baseURL *url.URL
	headers http.Header
	http    *http.Client
...
// NewClient creates a new Todostore API client.
...
func (c *Client) newRequest(method, path string, v interface{}) (*http.Request, error) {
	...
	case "PATCH", "POST":
		if v != nil {
			...
			log.Printf("[DEBUG] go-todostore body: " + string(dat))
...
func (c *Client) do(ctx context.Context, req *http.Request, v interface{}) error {
	...
	log.Printf("[DEBUG] go-todostore request: %v", req)

	// wake up the function?
	tempReq, _ := c.newRequest("GET", "todos", nil)
	c.http.Do(tempReq)
	...
	log.Printf("[DEBUG] go-todostore response: %v", resp)
